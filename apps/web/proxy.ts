...
export default async function proxy(request:NextRequest){
  ...
  if(!supabaseUrl||!anonKey){const denied=NextResponse.redirect(new URL(`/${locale}/login`,request.url));denied.headers.set('X-Platform-Auth-Mode','unavailable-fail-closed');return denied;}
