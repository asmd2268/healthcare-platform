...
import type {TransferCapabilities} from '@/features/inventory/mutations';
...
function InventoryHeader({title, body, readOnly = true}: {title: string; body: string; readOnly?: boolean}) {
  const t = useTranslations('inventory');
  return <><InventoryNav/><header className="inventory-heading"><div><p className="eyebrow">{t('eyebrow')}</p><h1>{title}</h1><p>{body}</p></div><Badge>{readOnly ? t('readOnly') : t('operationsEnabled')}</Badge></header></>;
}
...
export function InventoryTransfersView({transfers, filters, canCreate}: {transfers: InventoryTransfer[]; filters: {query: string; status: string}; canCreate: boolean}) {
  const t = useTranslations('inventory');
  const locale = useLocale() as Locale;
  ...
  return <section className="page inventory-page"><InventoryHeader title={t('transfers')} body={t('transfersBody')} readOnly={false}/>{canCreate ? <Link className="ui-button inventory-create-link" href={`/${locale}/inventory/transfers/new`}>{t('createTransfer')}</Link> : null}<FilterForm>
...
export function InventoryTransferDetailView({transfer, locations, capabilities}: {transfer: InventoryTransferDetail; locations: InventoryLocation[]; capabilities: TransferCapabilities}) {
  ...
  return <section className="page inventory-page"><InventoryNav/><Link className="inventory-back" href={`/${locale}/inventory/transfers`}>{t('backToTransfers')}</Link><header className="inventory-heading"><div><p className="eyebrow">{t('transfer')}</p><h1>{t('transferReference', {id: shortId(transfer.id)})}</h1><p>{localName(locale, transfer.sourceNameAr, transfer.sourceNameEn, t('restrictedLocation'))} → {localName(locale, transfer.destinationNameAr, transfer.destinationNameEn, t('restrictedLocation'))}</p></div><Badge>{t(`statuses.${transfer.status}`)}</Badge></header><p className="inventory-readonly-note">{t('detailSecurityNotice')}</p>
    ...
    </div><TransferActions transfer={transfer} locations={locations} capabilities={capabilities}/>
  </section>;
}
