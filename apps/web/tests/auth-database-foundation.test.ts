...
const profileAccessMigration=fs.readFileSync(path.join(process.cwd(),'../../supabase/migrations/202607270001_grant_authenticated_user_profile_read.sql'),'utf8');
...
describe('authentication fail-closed policy',()=>{
  ...
  it('does not reject configured protected routes before checking the verified user',()=>{expect(proxySource).toContain('if(!supabaseUrl||!anonKey)');expect(proxySource).not.toContain('authenticationRequired(Boolean(supabaseUrl&&anonKey),false)');});
...
describe('database foundation migration',()=>{
  ...
  it('grants authenticated users least-privilege access to their own profile',()=>{expect(profileAccessMigration).toContain('grant select on table public.user_profiles to authenticated');expect(profileAccessMigration).toContain('revoke select on table public.user_profiles from public, anon');expect(profileAccessMigration).toContain('alter policy profile_self on public.user_profiles');expect(profileAccessMigration).toContain('to authenticated');expect(profileAccessMigration).toContain('using (id = auth.uid())');});
});
