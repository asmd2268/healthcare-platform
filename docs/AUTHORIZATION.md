...
تطبق دالة `inventory_actor_has_permission` العقد نفسه على العمليات الموثوقة التي تنسب الفعل إلى مستخدم بشري: مالك المنصة العالمي الصحيح لا يحتاج membership لكل tenant، بينما أي دور آخر—even إذا كان `scope_level=global`—يبقى محتاجًا membership نشطًا ومتطابقًا مع tenant/organization/facility إضافة إلى الصلاحية المطلوبة. هذا الاستثناء الضيق يتيح لمالك المنصة تسجيل هوية أتمتة أو تعطيلها دون تحويل تعيينه العالمي إلى تعيين tenant مصطنع.
