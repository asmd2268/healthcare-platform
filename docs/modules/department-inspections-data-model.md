...
N/A صالح فقط إذا كان `allowNotApplicable` صحيحًا؛ وإلا تبقى الإجابة المطلوبة ناقصة. السؤال الاختياري غير المجاب لا يدخل المقام. القيم الرقمية السالبة مرفوضة، والقيمة الأعلى من `maxScore` تُقص إلى الحد الأعلى قبل الحساب. لا تنشأ مخالفة لسؤال حرج مطابق؛ تنشأ فقط لإجابة `isFailure` أو `createsFinding`.
