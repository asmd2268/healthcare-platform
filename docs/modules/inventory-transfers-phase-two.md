...
The read-only activity page exposes recent transfer events through the signed-in session and existing RLS policies; it does not create notifications or mutate event history. Delivery notifications, cross-facility transfers, supplier receipt/procurement, custody, controlled wastage approval, attachments/imports, CAPA adapters, printing, and reporting remain deferred. Legacy reservation-event backfill, standalone manual release, and correction/reversal adjustments remain deferred.
